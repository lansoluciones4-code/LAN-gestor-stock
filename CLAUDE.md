...
1. **Borrado físico en build-time**: `scripts/prepare-vercel-build.js`, ejecutado solo por `npm run build:public`, borra con `fs.rmSync` las carpetas `src/app/(main)` y `src/app/(auth)` **antes** de correr `next build`. El código de gestión ni siquiera queda bundleado. El build local (`npm run build`, usado por Docker/LAN) nunca corre este script — ahí sí se compila todo.
   ...
   ⚠️ **`fs.rmSync` borra del disco real del working directory, no de una copia de build.** Si un agente de IA corre `npm run build:public` (o `node scripts/prepare-vercel-build.js` directamente) para "probar" que el build público sigue aislando bien las rutas, **borra de verdad** `src/app/(main)` y `src/app/(auth)` del checkout en el que está parado. Los archivos ya commiteados se pueden recuperar sin pérdida (`git show HEAD:<archivo>` trae el contenido exacto, o `git checkout -- src/app/\(main\) src/app/\(auth\)`), pero **cualquier archivo nuevo sin commitear en esas carpetas se pierde para siempre** — no hay forma de recuperarlo. **Nunca corras `build:public` sobre el checkout principal para verificar algo**: hacelo en un `git worktree` aparte (`git worktree add ...`), o simplemente confiá en el análisis de la tabla de arriba ("Qué es de cada lado") en vez de ejecutar el borrado real para comprobarlo.
