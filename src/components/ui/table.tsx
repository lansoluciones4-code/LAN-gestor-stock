...
const Table = React.forwardRef<HTMLTableElement, TableProps>(
  ({ className, headers, isPending, children, ...props }, ref) => (
      ...
      >
        {headers && (
          <thead className="sticky top-0 z-10 text-sm uppercase bg-zinc-50 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400 shadow-sm">
            <tr>
              {headers.map((h, i) => (
                <th key={i} className="px-6 py-4 font-medium" scope="col">{h}</th>
              ))}
            </tr>
          </thead>
        )}
        {headers ? (
          <tbody className={`divide-y divide-zinc-200 dark:divide-zinc-800 ${isPending ? 'opacity-50 pointer-events-none' : ''}`}>
            {children}
          </tbody>
        ) : (
          children
