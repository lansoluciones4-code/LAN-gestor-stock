...
export function SalePaymentModal({ isOpen, onClose, subtotal, cards, allowedTypes = PAYMENT_TYPES, onConfirm, isPending }: SalePaymentModalProps) {
  ...
  const typedAmount = Number((amount || '0').replace(',', '.')) || 0;
  const remaining = total - typedAmount;
  ...
  const applyInstallmentOption = (opt: { installments: number; interestPercentage: number }) => {
    ...
    const computed = roundToDecimals(Math.max(total, 0) * (1 + opt.interestPercentage / 100));
    setAmount(computed.toFixed(2).replace('.', ','));
    setError(null);
  };

  // Recalcula el monto sugerido al total actualizado cada vez que cambia el % de descuento.
  const refillAmount = (newPct: number) => {
    const newTotal = roundToDecimals(subtotal * (1 - newPct / 100));
    setAmount(Math.max(newTotal, 0).toFixed(2).replace('.', ','));
  };
  ...
  const handleSubmit = () => {
    ...
    // En cuotas, se permite exceder el total (recargo por interés); en pago único, debe ser exacto.
    const allowExceed = type === 'credito' && installments > 1;
    if (allowExceed) {
      if (numAmount < total - 0.001) {
        setError(`En ${installments} cuotas, el monto no puede ser menor al total de la venta ($${total.toLocaleString('es-AR')})`);
        return;
      }
    } else if (Math.abs(numAmount - total) > 0.001) {
      setError(`El monto debe ser igual al total de la venta ($${total.toLocaleString('es-AR')})`);
      return;
    }

    onConfirm([{ type, amount: numAmount, installments: type === 'credito' ? installments : 1, cardId: type === 'credito' ? selectedCardId : null }], pVal);
  };
